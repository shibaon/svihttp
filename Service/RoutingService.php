...
class RoutingService extends AppContainer
{
    use BundleTrait;
	...
	public function getUrl($name, array $parameters = null, $absolute = false, $protocol = null)
	{
		...
		if ($absolute) {
			$route = $this->getHttpService()->getRequest()->getHttpHost() . $route;
			
			if (!in_array($protocol, ['https', 'http'])) {
				$protocol = $this->getHttpService()->getRequest()->isSecure() ? 'https' : 'http';
			}
